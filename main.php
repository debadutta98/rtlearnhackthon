...
'</h2> <table border="0" cellpadding="0" cellspacing="0" class="btn btn-primary"> <tbody> <tr> <td align="left"> <table border="0" cellpadding="0" cellspacing="0"> <tbody> <tr> <td>'."<a href='{$local_url}/otpmanager.php?email={$email}&otp={$otp}' target='_blank'>".'confirm email</a> </td> </tr> </tbody> </table> </td> </tr> </tbody> </table> <p>If you received this email by mistake, simply delete it. You willnot be subscribed if you do not click the confirmation link above.</p> </td> </tr> </table> </td> </tr> <!-- END MAIN CONTENT AREA --> </table> <!-- START FOOTER --> <div class="footer"> <table border="0" cellpadding="0" cellspacing="0"> <tr> <td class="content-block"> <span class="apple-link">rtlearn.com | Feminism | Culture | Law | Feminists Rising</span>'." <br>Do not like these emails? <a href='{$local_url}/unsubscrib.php?email={$email}'>".'Unsubscribe</a>. </td> </tr> <tr> <td class="content-block powered-by"> Powered by <a href="https://rtlearn.com">rtlearn</a>. </td> </tr> </table> </div> <!-- END FOOTER --> <!-- END CENTERED WHITE CONTAINER --> </div> </td> <td>&nbsp;</td> </tr> </table> </body></html>';
$id=uniqid().date("Ymdhhis");
$sql1="SELECT email, verify FROM user where email='$email'";
$result = $conn->query($sql1);
$row = $result->fetch_assoc();
if ($result->num_rows <=0) {
  $sql = "INSERT INTO user(Id, Name, email,password,verify,OTP) VALUES ('$id','$name','$email','$password','$valid',$otp)";
...
{
echo "<script> var i='{$local_url}/?massage=Check your connection';window.location.href=i; </script>";
}
}
else if($row["verify"]=='0' || $row["verify"]==1)
{

echo "<script> var i='{$local_url}/?massage=user already exit please login';window.location.href=i; </script>";
}
else
{
    echo 404;
